...
- **🚨 CLOUDFLARE WORKERS BUILDS CI RACE (recurred twice, 6–7 Aug 2026, fixed 7 Aug 2026).** The GitHub repo was connected to Cloudflare's own "Workers Builds" (Workers → leadenthrella → Settings → Build → Git repository), which auto-builds on every push, independently of `scripts/ship.sh`. Its build had no `.env` and no dashboard-side "Variables and secrets" for `VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY`, so it produced the exact same MISSING-ENV TRAP above — and it deployed ~20-60s *after* the manual `ship.sh` deploy, silently overwriting the good one. **Harish disconnected the repo on 7 Aug 2026 — `scripts/ship.sh` is now the only deploy path, full stop.** Do not reconnect Workers Builds; if it's ever needed again, add both `VITE_` vars under Settings → Build → Variables and secrets first or this recurs immediately.
